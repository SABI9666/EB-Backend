...
// Multer: memory storage for variation file upload
const upload = multer({
...
const handler = async (req, res) => {
    try {
        // ============================================
        // POST - Create a new accounts-uploaded variation
        // ============================================
        if (req.method === 'POST') {
            return upload.single('variationFile')(req, res, async (multerErr) => {
                ...
                // Only Accounts (and COO/Director for admin overrides) can upload
                if (!['accounts', 'coo', 'director'].includes(req.user.role)) {
                    return res.status(403).json({
                        success: false,
                        error: 'Only Accounts users can upload account variations.'
                    });
                }
                ...
                if (!bdmUid || !bdmName) {
                    return res.status(400).json({
                        success: false,
                        error: 'BDM (bdmUid + bdmName) is required.'
                    });
                }
                if (variationValue === undefined || variationValue === null || variationValue === '') {
                    return res.status(400).json({
                        success: false,
                        error: 'Variation value is required.'
                    });
                }
                ...
                if (isNaN(parsedValue) || parsedValue < 0) {
                    return res.status(400).json({
                        success: false,
                        error: 'Variation value must be a non-negative number.'
                    });
                }

                // Verify BDM user exists and has bdm role
                ...
                if (bdmData.role !== 'bdm') {
                    return res.status(400).json({
                        success: false,
                        error: 'Selected user is not a BDM.'
                    });
                }
                ...
                if (req.file) {
                    ...
                    await fileRef.makePublic().catch(e =>
                        console.log('Note: Could not make account-variation file public:', e.message)
                    );
                ...
                const docData = sanitizeForFirestore({
                    bdmUid: bdmUid,
                    bdmName: bdmName,
                    bdmEmail: bdmEmail || bdmData.email || null,
                ...
                await db.collection('activities').add({
                    ...
                    accountVariationId: ref.id,
                    bdmUid: bdmUid,
                    bdmName: bdmName
                }).catch(e => console.warn('account-variation activity log failed:', e.message));
                ...
                // Notify all COOs so it appears in their Variation Tracker
                try {
                    ...
                    console.warn('COO notification failed:', e.message);
                }

                return res.status(200).json({
                    success: true,
                    message: 'Account variation uploaded successfully.',
                    id: ref.id
                });
            });
        }
        ...
        // Parse JSON body for PUT / DELETE-with-body
        ...
        if (req.method === 'GET') {
            ...
            if (id) {
                ...
                if (!doc.exists) {
                    return res.status(404).json({ success: false, error: 'Account variation not found.' });
                }
                const data = doc.data();
                // BDMs can only view their own
            ...
            if (role === 'bdm') {
                ...
                query = query.where('bdmUid', '==', bdmUid);
            }
            ...
            try {
                ...
                // Fallback if composite index isn't ready: drop orderBy
                snapshot = await query.get();
            }

            const list = snapshot.docs.map(d => ({ id: d.id, ...d.data() }));
            // Manual sort by createdAt desc (safe fallback)
            list.sort((a, b) => {
        ...
        // ============================================
        // PUT - Update an existing account variation
        // ============================================
        if (req.method === 'PUT') {
            const { id } = req.query;
            if (!id) {
            ...
            const updates = sanitizeForFirestore({
                variationValue: body.variationValue !== undefined
                    ? parseFloat(body.variationValue)
                    : undefined,
        ...
        // ============================================
        // DELETE - Remove an account variation (and file)
        // ============================================
